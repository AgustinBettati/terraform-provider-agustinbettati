...
- `api_key` (String, Sensitive) Applicable when type is: DATADOG_LOG_EXPORT. API key for authentication.
- `bucket_name` (String) Applicable when type is: GCS_LOG_EXPORT, S3_LOG_EXPORT. Name of the bucket to store log files.
- `hec_token` (String, Sensitive) Applicable when type is: SPLUNK_LOG_EXPORT. HTTP Event Collector (HEC) token for authentication.
- `hec_url` (String) Applicable when type is: SPLUNK_LOG_EXPORT. HTTP Event Collector (HEC) endpoint URL.
- `iam_role_id` (String) Applicable when type is: S3_LOG_EXPORT. Unique 24-character hexadecimal string that identifies the AWS IAM role that Atlas uses to access the S3 bucket.
...
- `otel_endpoint` (String) Applicable when type is: OTEL_LOG_EXPORT. OpenTelemetry collector endpoint URL.
- `otel_supplied_headers` (Attributes List, Sensitive) Applicable when type is: OTEL_LOG_EXPORT. HTTP headers for authentication and configuration. Maximum 10 headers, total size limit 2KB. (see [below for nested schema](#nestedatt--results--otel_supplied_headers))
- `prefix_path` (String) Applicable when type is: AZURE_LOG_EXPORT, GCS_LOG_EXPORT, S3_LOG_EXPORT. Path prefix where the log files will be stored. Atlas will add further sub-directories based on the log type.
- `region` (String) Applicable when type is: DATADOG_LOG_EXPORT. Datadog site/region for log ingestion. Valid values: US1, US3, US5, EU, AP1, AP2, US1_FED.
- `role_id` (String) Applicable when type is: AZURE_LOG_EXPORT, GCS_LOG_EXPORT. Unique 24-character hexadecimal string that identifies the Atlas Cloud Provider Access role.
- `storage_account_name` (String) Applicable when type is: AZURE_LOG_EXPORT. Storage account name where logs will be stored.
- `storage_container_name` (String) Applicable when type is: AZURE_LOG_EXPORT. Storage container name for log files.
